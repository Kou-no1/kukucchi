...
import type { SubtractionMonsterDefinition } from '../../data/subtractionMonsters'

function classNames(...values: Array<string | undefined | false>): string {
  return values.filter(Boolean).join(' ')
}

function motifCells(monster: SubtractionMonsterDefinition): Array<{ x: number; y: number }> {
  if (monster.motif === 'split') {
    return [
      { x: 5, y: 3 },
      { x: 6, y: 4 },
      { x: 5, y: 5 },
  ...
  if (monster.motif === 'borrow' || monster.motif === 'large') {
    return [
      { x: 3, y: 5 },
      { x: 4, y: 5 },
      { x: 5, y: 5 },
...
function buildBodyCells(monster: SubtractionMonsterDefinition): Array<{ x: number; y: number }> {
  ...
  for (let y = 2; y <= 11; y += 1) {
    for (let x = 2; x <= 10; x += 1) {
      ...
      const chipped =
        (monster.motif === 'chip' && x >= 8 && y <= 4) ||
        (monster.motif === 'split' && x === 6 && y >= 3 && y <= 9) ||
        (monster.motif === 'borrow' && x <= 3 && y >= 8)
      if (normalized <= 1 && !chipped) {
        cells.push({ x, y })
      }
    }
  }
...
export const SubtractionMonsterSprite = memo(function SubtractionMonsterSprite({
  monster,
  locked = false,
  className,
}: {
  ...
  return (
    <svg
      className={classNames('monster-pixel-sprite subtraction-monster-sprite', className)}
      viewBox="0 0 104 104"
      role="img"
      aria-label={locked ? 'みかくとくひきざんもんすたー' : monster.name}
    ...
    >
      ...
      {body
        ...
        .map((cell) => renderCell(cell, colors.shadow, `shadow-${cell.x}-${cell.y}`, locked ? 0.36 : 0.48))}
      {body
